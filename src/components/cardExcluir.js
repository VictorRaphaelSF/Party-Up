...
export default function CardExcluir(props) {
  const navigation = useNavigation();

  const handleButtonExcluir = () => {
    if (props.nomeEvento === props.titulo) {
    const idDeEvento = {
      idEvent: props.idEvento,
    };
    axios
      .post("http://localhost:3003/deleteEvent", idDeEvento)
      .then((response) => {
        console.log(response);
        navigation.navigate("telaprincipal", { id: 1 });
        ...
        <View style={styles.containerExcluir}>
          <View style={styles.contentExcluir}>
            <Text style={styles.excluirLabel}>
              Digite o nome do evento para excluí-lo
            </Text>
            <Animatable.View style={styles.menuContainer} duration={250}>
              <View style={styles.containerBtns}>
                <TextInput
                  ...
                  onChangeText={props.setNomeEvento}
                />
                <Pressable
                  style={styles.excluirBtn}
                  onPress={handleButtonExcluir}>
                  <Text style={styles.excluirText}>Excluir</Text>
                </Pressable>
                <Pressable
                  style={styles.cancelBtn}
                  onPress={() => props.setExcluir(false)}>
